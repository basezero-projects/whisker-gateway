...
vi.mock("@/lib/engine-client", () => ({
  callEngine: vi.fn(async () => [{ id: 1, title: "hi" }]),
}));

const { GET } = await import("@/app/api/v1/transcripts/route");

describe("GET /api/v1/transcripts", () => {
  it("happy path", async () => {
    const req = new Request("http://x/api/v1/transcripts", {
